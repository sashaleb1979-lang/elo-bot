...
async function getFreshDiscordAvatarUrl(client, userId) {
  if (!client || !userId) return "";
  const user = await client.users.fetch(userId).catch(() => null);
  if (!user) return "";
  return normalizeDiscordAvatarUrl(user.displayAvatarURL({ extension: "png", forceStatic: true, size: 256 }));
}

async function loadGraphicAvatarForPlayer(client, player) {
  const existingUrl = normalizeDiscordAvatarUrl(player?.avatarUrl || "");
  let avatar = existingUrl ? await loadGraphicAvatar(existingUrl) : null;
  if (avatar) return avatar;

  const freshUrl = await getFreshDiscordAvatarUrl(client, player?.userId);
  if (!freshUrl || freshUrl === existingUrl) return null;

  avatar = await loadGraphicAvatar(freshUrl);
  if (!avatar) return null;

  player.avatarUrl = freshUrl;
  const rating = db.ratings?.[player.userId];
  if (rating) {
    rating.avatarUrl = freshUrl;
...
async function hydrateGraphicAvatarUrls(client) {
  ...
  for (const [userId, rating] of Object.entries(db.ratings || {})) {
    ...
    if (current && isDiscordCdnUrl(current)) {
    ...
    const fresh = await getFreshDiscordAvatarUrl(client, userId);
    if (!fresh) continue;
